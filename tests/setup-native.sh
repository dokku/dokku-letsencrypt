...
# The first few `apps:create` invocations on a freshly-bootstrapped dokku host
...
warmup_attempts=0
until sudo dokku apps:create letest-warmup >/dev/null 2>&1; do
...
sudo dokku --force apps:destroy letest-warmup >/dev/null 2>&1 || true
